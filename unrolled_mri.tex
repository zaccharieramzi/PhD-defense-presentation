        ...
        \begin{tikzpicture}[
            ...
            \only<3->{
                ...
                \only<6->{
                    ...
                    \node (smaps_exp) [below=0.4em of smaps_refiner] {U-Net\footfullcite{ronneberger2015u}};
                    % smaps arrows
                ...
                \begin{scope}[on background layer]
                    ...
                    %% highlighting nodes
                    \node<4> (highlight_concat) [highlight_node, fit=(backward_op) (dc_input_comb)] {};
                    \node<5> (highlight_mwcnn) [highlight_node, fit=(prox_exp)] {};
                    \node<6> (highlight_smaps) [highlight_node, fit=(smaps_refiner) (smaps) (smaps_exp)] {};
                \end{scope}
                % arrows linking all nodes
                %% main track
...
\begin{frame}{Robustness test}
    ...
    \begin{figure}[h]
        ...
        \end{tabular}
        \caption{\textbf{XPDNet reconstruction on a brain prospectively accelerated.\footfullcite{Marrakchi-Kacem2016RobustChoices}} (zoom on the cerebellum) \label{fig:brain-7t}}
        \end{center}
    \end{figure}
\end{frame}
...
\begin{frame}{NC-PDNet - 1}
    ...
    \begin{center}
        \begin{tikzpicture}[
            ...
            % XXX pot change black to other color for unit node
            % Unrolled net simple
            \node (input) {$\xb_0$};
            % Unrolled net exploded
            % nodes
            ...
            \node (input_forward) [left=3em of forward_op] {};
            \node (dc) [kspace_node, above right=of forward_op] {$\cdot - \ybb$};
            \node (dcp) [kspace_node, right=of dc, visible on=<2->] {$\cdot \times$ DCp};
            ...
            \end{scope}
            %% smaps nodes
            \node (smaps_refiner) [opnode, below left=of forward_op] {Smaps refiner};
            \node (smaps) [left=of smaps_refiner] {$\Sbb$};              
            %% highlight nodes
