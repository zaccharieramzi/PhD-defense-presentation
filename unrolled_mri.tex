...
\begin{frame}{Model agnostic learning}
    ...
    \begin{tikzpicture}[overlay,remember picture,>=stealth,nodes={align=left,inner ysep=1pt},<-]
        % For "kspace"
        \onslide<2->{
        \path (kspace.south) ++ (0,-7.3em) node[anchor=south east,color=amber!87] (exp_kspace){
            \textbf{kspace} \includegraphics[width=0.15\textwidth]{Figures/intro_figures/kspace_mri.png}};
        \draw [color=amber!87](kspace.south) |- ([xshift=-0.3ex,color=amber]exp_kspace.south west);
        }
        % For "image"
        \onslide<3->{
        \path (image.south) ++ (0, -6.3em) node[anchor=south west,color=blue!87] (exp_image){
            \includegraphics[width=0.15\textwidth,trim=3.3em 3.3em 3.3em 3.3em,clip]{Figures/dl_mri_figures/gt_knee.png} \textbf{image}};
    ...
    \begin{itemize}
        ...
        \item No scaling (larger images, multi-coil, 3D)
    \end{itemize}
    }
\end{frame}
...
\begin{frame}{Single domain learning}
    ...
    \begin{tikzpicture}[overlay,remember picture,>=stealth,nodes={align=left,inner ysep=1pt},<-]
        ...
        % For aliased image
        \onslide<2>{
        \path (kspace.south) ++ (0, -6.3em) node[anchor=south east,color=amber!87] (exp_aliased_image){
            \textbf{aliased image} \includegraphics[width=0.15\textwidth,trim=3.3em 3.3em 3.3em 3.3em,clip]{Figures/dl_mri_figures/zfilled_recon_af4.png}};
...
\begin{frame}{Unrolled models - 2~[Ramzi et al. 2020a]}
...
\begin{frame}{Unrolled models - 2~[Ramzi et al. 2020a]}
    ...
    \vspace{-1em}
        ...
        \footnotetext{$^9$\fullcite{Adler2018}}
...
\begin{frame}
    \frametitle<1-4>{PDNet}
    \frametitle<5->{XPDNet~[Ramzi et al. 2020b]}
    ...
    \begin{center}
        \begin{tikzpicture}[
            ...
            \only<3->{
                ...
                \node (prox_exp_cnn) [below=2em of prox, visible on=<3-5>] {CNN};
                \node (prox_exp_mwcnn) [below=2em of prox, visible on=<6->] {MWCNN\footfullcite{Liu2018}};
                \node (outpt_prox) [right=3em of prox] {$\xb_1$};
                % smaps nodes
                \node (smaps_refiner) [opnode, below left=of forward_op, visible on=<7->] {Smaps refiner\footfullcite{Sriram2020End-to-EndReconstruction}};
                \node (smaps) [left=of smaps_refiner, visible on=<7->] {$\Sbb$};
                \node (smaps_exp) [below=0.4em of smaps_refiner, visible on=<7->] {U-Net\footfullcite{ronneberger2015u}};
                \begin{scope}[on background layer]
                    ...
                    %% highlighting nodes
                    \node (highlight_concat) [highlight_node, fit=(backward_op) (dc_concat) (dc_res), visible on=<3-4>] {};
                    \node (highlight_mwcnn) [highlight_node, fit=(prox_exp_mwcnn), visible on=<5-6>] {};
                    \node (highlight_smaps) [highlight_node, fit=(smaps_refiner) (smaps) (smaps_exp), visible on=<7>] {};
                \end{scope}
                % arrows linking all nodes
                %% main track
                ...
                \draw<3-5> [line width=0.1em] (prox.south) -- (prox_exp_cnn.north);
                \draw<6-> [line width=0.1em] (prox.south) -- (prox_exp_mwcnn.north);
                % smaps arrows
                \draw<7-> [<-] (smaps_refiner) -- (smaps);
                \draw<7-> [->] (smaps_refiner) -| (forward_op);
                \draw<7-> [->] (smaps_refiner) -| (simple_backward_op);
                \draw<7-> [line width=0.1em] (smaps_refiner.south) -- (smaps_exp.north);
            }
...
\begin{frame}
    ...
    \only<1>{
        \begin{tabular}{c@{\hspace*{\qualifigsepigogd}}c@{\hspace*{\qualifigsepigogd}}c}
            {\textbf{Reference}} & \makecell{{\textbf{GRAPPA }} \\ PSNR: 26 dB, SSIM: 0.77} & \makecell{{\textbf{XPDNet}} \\ PSNR: 36 dB, SSIM: 0.96} \\
    ...
    \only<2>{
        \begin{tabular}{c@{\hspace*{\qualifigsepigogd}}c@{\hspace*{\qualifigsepigogd}}c}
            {\textbf{Reference}} & \makecell{{\textbf{GRAPPA }} \\ PSNR: 26 dB, SSIM: 0.77} & \makecell{{\textbf{XPDNet}} \\ PSNR: 36 dB, SSIM: 0.96} \\
            \includegraphics[width=\thefigdimigogd\textwidth,trim=250 200 200 250, clip]{Figures/clinic_applic/fastmri_r8/GT_Pinf_S1.0000.png}&
...
\begin{frame}{fastMRI challenge~[Ramzi et al. 2020b], [Muckley et al. 2021]}
...
\begin{frame}{Non-Cartesian acquisitions}
    ...
    Non-Cartesian acquisitions more robust to motion.
    \pause
    \vspace{-1ex}
    \begin{figure}
        \centering
    ...
    \textbf{Nonuniform Fourier Transform (NDFT)} too costly $\Rightarrow$ NUFFT, with our TensorFlow implementation:
    % We resort to using its fast approximation, the NUFFT, which we implemented in TensorFlow, to enable gradient-based learning:
    \begin{itemize}
        \item \faPython \thickspace \highlight{black}{\texttt{pip install tfkbnufft}}
    \end{itemize}
\end{frame}

\begin{frame}{NC-PDNet - 1~[Ramzi et al. 2022a]}
...
\begin{frame}{NC-PDNet - 2~[Ramzi et al. 2022a]}
    % give results
...
\begin{frame}
    \begin{figure}
        % \begin{overprint}
            \only<1>{\centering\includegraphics[height=0.9\textheight]{Figures/dl_mri_figures/single_coil.pdf}\caption{\textbf{2D single-coil reconstruction quantitative results on the fastMRI knee dataset.}}}
            \only<2>{\centering\includegraphics[width=\textwidth]{Figures/dl_mri_figures/quali_no_err_af4_radial.pdf}\caption{\textbf{2D single-coil reconstruction qualitative results on the fastMRI dataset for a radial trajectory.}}}
            \only<3>{\centering\includegraphics[width=0.7\textwidth]{Figures/dl_mri_figures/3d.pdf}\caption{\textbf{3D single-coil reconstruction quantitative results on the OASIS dataset for a radial trajectory.}}}
            \only<4>{\centering\includegraphics[width=\textwidth]{Figures/dl_mri_figures/quali_no_err_3d_af4_radial.pdf}\caption{\textbf{3D single-coil reconstruction qualitative results on the OASIS dataset for a radial trajectory.}}}
        % \end{overprint}
    \end{figure}

    \begin{tikzpicture}[overlay,remember picture,>=stealth,nodes={align=left,inner ysep=1pt},<-]
        \draw[draw=red!47, dashed, visible on=<2>, line width=0.5mm] (12.2cm,1.48cm) rectangle ++(2.3cm,4.5cm);
        \draw[draw=red!47, dashed, visible on=<4>, line width=0.5mm] (10.7cm,1.4cm) rectangle ++(3.8cm,7.5cm);
    \end{tikzpicture}
\end{frame}
