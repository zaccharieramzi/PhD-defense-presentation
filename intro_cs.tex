...
\begin{frame}{Application to MRI}
    ...
    \begin{tikzpicture}[overlay,remember picture,>=stealth,nodes={align=left,inner ysep=1pt},<-]
        ...
        % For "x"
        \onslide<2->{
        \path (x.south) ++ (0, -4em) node[anchor=south west,color=blue!67] (exp_x){\includegraphics[trim=3.3em 3.3em 3.3em 3.3em, clip,width=0.08\textwidth,angle=180]{Figures/intro_figures/brain_mri.png} \textbf{2D or 3D MR image $\in \mathbb{C}$}};
    ...
    \onslide<4>{
        ...
        $\Sbb$: \includegraphics[height=0.2\textheight]{Figures/cs_figures/smaps.png}
    }
\end{frame}
