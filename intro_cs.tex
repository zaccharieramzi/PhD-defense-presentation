...
\begin{frame}{Another look at redundancy: the prior point of view}
    ...
    \only<2>{
        \begin{center}
            \begin{tikzpicture}[font=\large,>=stealth,node distance=1.5em]
                % node
                \node(f_smiley) {$f(\adjincludegraphics[valign=c,width=0.1\textwidth]{Figures/intro_figures/smiley_prior_x.pdf}) = $};
                ...
                % arrow
                % \draw[->,-{Triangle[width=18pt,length=9pt]}, line width=7pt, shade, bottom color=red, top color=green]  (unlikely_smiley.north) -- (likely_smiley.south);
            \end{tikzpicture}
        \end{center}
    }
\end{frame}
