...
\begin{frame}{Single domain learning}
    ...
    \begin{equation*}
        \alt<1>{\mathcal{A}^H}{}f_{\thetab}(\tikzmarknode{aliased_image}{\alt<2>{\mathcal{A}^H}{}\highlight{yellow}{$\ybb$}}) = \highlight{blue}{$\xb$}
    \end{equation*}

    \begin{tikzpicture}
        % For "aliased_image"
        \onslide<4->{
        \path (aliased_image.south) ++ (0, -2em) node[anchor=south west,color=yellow!87] (exp_aliased_image){
            \textbf{aliased_image}};
