...
\begin{frame}{Physics of MRI - 2}
    ...
    \end{equation*}
    % XXX: use onslide in tikzpicture
...
\begin{frame}{Formalism - 1}
    ...
    % XXX: have the sum in text be like in eq
    The classical framework for DL is supervised learning:
    \begin{equation*}
        \label{eq:dl-objective}
        \argmin_{\thetab \in \Theta} \tikzmarknode{sum}{\highlight{purple}{$\sum_{(\xb_i, \yb_i) \in \mathcal{D}}$}} \tikzmarknode{loss}{\highlight{green}{$\mathcal{L}$}}(\tikzmarknode{nn}{\highlight{brown}{$f_{\thetab}$}} (\tikzmarknode{input}{\highlight{blue}{$\xb_i$}}), \tikzmarknode{label}{\highlight{red}{$\yb_i$}}, \thetab)
    \end{equation*}
    % XXX: use onslide in tikzpicture
    \begin{tikzpicture}[overlay,remember picture,>=stealth,nodes={align=left,inner ysep=1pt},<-]
        % For "prox"
